...
2. Bump the version in both manifests: `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`. The gate fails if they disagree, so a half-done bump cannot ship.

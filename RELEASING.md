...
2. Bump the version in all three manifests: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, and `.kimi-plugin/plugin.json`. The gate fails if they disagree, so a half-done bump cannot ship.

...
| ------- | ------- | ------------------- | ------ |
| Claude Code | `.claude-plugin/` (plugin manifest + `marketplace.json`) | SessionStart hook → `hooks/claude-code/context.md` (re-fires on clear and **compaction**, so the nudge survives a compacted session) | Exercised — evidence in `tests/harness/claude-code.md` |

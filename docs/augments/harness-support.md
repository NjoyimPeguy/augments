...
| Codex | `plugins/augments/.codex-plugin/`, `.agents/plugins/marketplace.json`, and project-local `.codex/hooks.json` | Installed skill catalogue plus a project-local Stop nudge; durable repository guidance comes from `AGENTS.md`; no per-prompt reminder is shipped |
...
2. Put the pointer from `scripts/sh/session-start.sh` in a durable instruction
   surface so the agent reaches for `using-augments`.
...
Where the harness exposes a blockable edit event, augments keeps a small guard
...
Wide migrations are routed by `using-augments` to `migration-strategy` and

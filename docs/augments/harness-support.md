...
The Claude manifest must list every skill on disk, and the Codex adapter must expose a flat generated mirror for every canonical skill. The gate fails CI if either adapter drifts (see *Adding a harness adapter*).

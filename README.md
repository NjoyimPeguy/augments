...
On Kimi Code, augments ships a plugin manifest (`.kimi-plugin/plugin.json`) whose `sessionStart.skill` loads the `using-augments` router into every new and resumed session, whose `skillInstructions` bind the skills' tool language to Kimi's real tools whenever a plugin skill loads, and whose declared Stop hook re-nudges at the done boundary (`hooks/stop-nudge-kimi.sh`, sharing its detection policy with the other harnesses). The manifest points at the canonical phase directories directly — no mirror. The known gap: the session-start nudge does not survive mid-session compaction (see `tests/kimi-code/README.md`).

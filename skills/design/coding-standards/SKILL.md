...
1. **Fix the domain vocabulary.** One canonical term per concept ("order", not order/purchase/transaction). Ban generic labels ("service", "manager", "handler", "util") where a domain term exists — the model treats synonyms as distinct things, so this is a design decision, not pedantry.
...
5. **Write the coding-standards section** of the design document, or a standing conventions file — default `docs/augments/designs/{{YYYY-MM-DD}}-{{topic}}.md`.

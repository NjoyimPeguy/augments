...
5. **Choose an intentional direction.** For any open visual direction, read [design-quality.md](design-quality.md). Define layout, type, color, spacing, shape, imagery, and motion as one product-specific system; do not decorate around an unresolved hierarchy.
6. **Show alternatives only when seeing helps.** For a spatial, visual, or motion decision, read [visual-decisions.md](visual-decisions.md) and compare 2–4 controlled, meaningfully different variants. Keep conceptual requirements and trade-offs in conversation. If feasibility, not preference, is uncertain, invoke `prototyping`.

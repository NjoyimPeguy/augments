# tests/kimi-code/ - Kimi Code plugin adapter checks
...
needed. `scripts/sh/validate-kimi-plugin.sh` checks that the exposed set stays
...
```bash
bash scripts/sh/validate-kimi-plugin.sh
bash tests/kimi-code/run-activation.sh selftest
bash tests/kimi-code/test-stop-nudge.sh
bash tests/kimi-code/run-activation.sh \

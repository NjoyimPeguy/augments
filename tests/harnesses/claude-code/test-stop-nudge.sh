#!/usr/bin/env bash
# Deterministic, offline test for the Stop-hook done-boundary re-nudge — NO API call.
# The hook reads only the Stop payload (stop_hook_active + last_assistant_message), so
# each case is just a crafted payload; there is no transcript to fixture. A live
...
expect_block() { local out; out="$(run "$2" "$3")"
  if printf '%s' "$out" | grep -q '"decision":"block"'; then printf 'ok    %-22s re-nudges\n' "$1"
  else printf 'FAIL  %-22s expected block, got: %s\n' "$1" "${out:-<empty>}"; fail=1; fi; }

expect_allow() { local out; out="$(run "$2" "$3")"
  if [ -z "$out" ]; then printf 'ok    %-22s lets turn end\n' "$1"
  else printf 'FAIL  %-22s expected allow, got: %s\n' "$1" "$out"; fail=1; fi; }

echo "== stop-nudge detection (offline, payload-only) =="
...
if [ "$fail" -eq 0 ]; then echo "PASS — fires only on an un-looped completion claim"; else echo "FAILED"; exit 1; fi

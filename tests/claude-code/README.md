# tests/claude-code/ — real activation tooling
...
tests/claude-code/run-flow.sh --flow scenarios/planning/_flow --keep
tests/claude-code/run-flow.sh --flow scenarios/planning/_flow --print   # parse only, no API call

# A single cross-cutting skill:
tests/claude-code/run-activation.sh \
...
```bash
tests/claude-code/run-behavioral.sh --scenario spec-it --arm green --keep
tests/claude-code/run-behavioral.sh --scenario spec-it --arm red --base origin/dev
```

Scenarios are **shared across adapters** — they live in
`../../scenarios/behavioral/` and Codex CLI and Kimi Code have sibling
...
harness constraint. See `../../scenarios/behavioral/README.md`.
...
(`bash ../../scenarios/behavioral/spec-it/probe.sh <dir>`) — that checks the probe

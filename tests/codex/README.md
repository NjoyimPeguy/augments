# tests/codex/ - Codex plugin adapter checks
...
- `test-stop-nudge.sh` is an offline payload test for the Codex Stop hook
  ...
  `hooks/stop-nudge.sh`.
- `run-behavioral.sh` runs a shared scenario from `../scenarios/behavioral/` on Codex
...
`scripts/sh/sync-codex-plugin-skills.sh` and checked by `scripts/sh/validate-codex-plugin.sh`.

## Run it

```bash
bash scripts/sh/validate-codex-plugin.sh
bash tests/codex/run-plugin-smoke.sh
bash tests/codex/run-activation.sh selftest
bash tests/codex/test-stop-nudge.sh
bash tests/codex/run-activation.sh \
...
gate in `scripts/sh/validate-skills.sh`.

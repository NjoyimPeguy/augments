...
Kept deliberately as an honest guardrail, not a proven corrective. Its plausible value is in the *untested* regime — smaller models and long agentic contexts, where sycophancy and unverified deference are known to degrade. Like `verifying-completion`, it encodes the correct standard and costs little; but per `docs/augments/philosophy.md`, the reliable enforcement is the review process itself (a human or gating reviewer), not this nudge.

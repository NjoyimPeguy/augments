...
*Update (2026-06-21 · firmed to an imperative trigger):* the `description` was rewritten from "Use before…" to "ALWAYS invoke before claiming work is complete … 'should work' without a run is the mistake …", relocating firing pressure into the always-loaded catalogue (which does not decay over a long session). Routing re-measured: a done-claim opening ("it should be working now, let me mark it done and open the PR") routes **3/3 → `verifying-completion`**, intact (Claude Code, large-tier). Over-fire here is largely benign — verifying before *every* done-claim is the intent. Maintainer accepted the firmer trigger.
